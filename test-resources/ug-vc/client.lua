...
local MODEL_IMAGE_PATH = "assets/vc_models.img"
local TEXTURE_IMAGE_PATH = "assets/vc_textures.img"
local CITY_ID = "vc"
...
local function releaseRadar()
    if engineResetRadarMapTile then
        for _, tile in ipairs(UG_VC_RADAR_TILES) do
...
local function loadRadar()
    ...
    if not engineSetRadarMapTile or not engineGetRadarMapStats then
        outputChatBox("[UG VC radar] API radar Neon indisponible.", 255, 180, 80)
        return false
    end
    if not engineGetRadarMapStats().hooksInstalled then
        outputChatBox("[UG VC radar] Hooks radar inactifs sur cet executable.", 255, 80, 80)
        return false
    end
    ...
    for _, tile in ipairs(UG_VC_RADAR_TILES) do
        ...
        if not txd then
            releaseRadar()
            outputChatBox("[UG VC radar] ECHEC engineLoadTXD: " .. tile.path, 255, 80, 80)
            return false
        end
        if not engineSetRadarMapTile(tile.column, tile.row, txd) then
            destroyElement(txd)
            releaseRadar()
            outputChatBox(("[UG VC radar] ECHEC cellule %d,%d"):format(tile.column, tile.row), 255, 80, 80)
            return false
        end
        loadedRadarTiles[tile.path] = txd
    end

    local message = radarStatsMessage(("[UG VC radar] %d tuiles chargees;"):format(#UG_VC_RADAR_TILES))
    outputChatBox(message, 80, 220, 255)
    outputDebugString(message)
    return true
end

addCommandHandler("vcradarstats", function()
    local message = radarStatsMessage("[UG VC radar]")
    outputChatBox(message, 80, 220, 255)
    outputDebugString(message)
end)
...
local function buildIndexes()
    for sourceId in pairs(UG_VC_MODELS) do
        modelOrder[#modelOrder + 1] = sourceId
    end
    table.sort(modelOrder)

    for placementIndex, placement in ipairs(UG_VC_PLACEMENTS) do
...
local function requestTexture(path)
    ...
    local slotName = "ugvc_" .. entryName:gsub("%.[^.]+$", "")
...
local function registerModel(sourceId)
    ...
    local model = UG_VC_MODELS[sourceId]
    if not model then
...
local function linkPlacementLod(placementIndex)
    local placement = UG_VC_PLACEMENTS[placementIndex]
...
local function createPlacement(placementIndex)
    ...
    local model = UG_VC_MODELS[placement.model]
...
local function reportReady()
    ...
    outputChatBox("[UG VC] Ville enregistree. /vctest pour Vice City.", 80, 255, 160)
    outputDebugString("[UG VC IMG] ready " .. details)
    triggerServerEvent("ugVcClientReady", resourceRoot, true, details)
...
failBootstrap = function(reason)
    ...
    removeEventHandler("onClientPreRender", root, processBootstrap)
    outputChatBox("[UG VC] ECHEC IMG: " .. reason, 255, 80, 80)
    outputDebugString("[UG VC IMG] " .. reason, 1)
    triggerServerEvent("ugVcClientReady", resourceRoot, false, reason)
    if prepareRequest then
        triggerServerEvent("ugVcPositionReady", resourceRoot, prepareRequest.token, false, reason)
        prepareRequest = nil
    end
    fadeCamera(true, 0.5)
end
...
function processBootstrap()
    ...
    while getTickCount() - startedAt < WORK_BUDGET_MS do
        ...
        elseif bootstrapStage == "placements" then
            if placementCursor > #UG_VC_PLACEMENTS then
                reportReady()
                return
            end
...
local function beginBootstrap()
    ...
    outputChatBox("[UG VC] Activation locale rapide...", 255, 200, 80)
    outputDebugString("[UG VC IMG] bootstrap " .. details)
end
...
local function reportPlacement(placementIndex, distance, chat)
    ...
    outputConsole("[UG VC inspect] " .. description)
    outputDebugString("[UG VC inspect] " .. description)
    if chat then
        outputChatBox("[UG VC] " .. description, 160, 220, 255)
    end
end

addCommandHandler("vcinspect", function(_, radiusArgument)
    ...
    outputChatBox(("[UG VC] %d instances dans %.0fm; les 12 plus proches sont dans F8."):format(#nearby, radius), 80, 255, 160)
...
addCommandHandler("vcinspectaim", function()
...
addCommandHandler("vcstreamstats", function()
    ...
    outputChatBox("[UG VC IMG] " .. message, 80, 220, 255)
    outputDebugString("[UG VC IMG] " .. message)
end)
...
finishPrepare = function()
    ...
    if enginePreloadWorldArea then
        ...
        if not succeeded then
            triggerServerEvent("ugVcPositionReady", resourceRoot, request.token, false, tostring(result))
            prepareRequest = nil
            fadeCamera(true, 0.5)
            return
        end
    ...
    outputDebugString("[UG VC IMG] prepare ready " .. details)
    triggerServerEvent("ugVcPositionReady", resourceRoot, request.token, true, details)
end

addEvent("ugVcPreparePosition", true)
addEventHandler("ugVcPreparePosition", resourceRoot, function(x, y, z, token)
    prepareRequest = {
        x = tonumber(x) or 0,
        y = tonumber(y) or 0,
        z = tonumber(z) or 0,
    ...
    if bootstrapStage == "ready" then
        prepareRequest.timer = rememberTimer(setTimer(finishPrepare, 350, 1))
        outputChatBox("[UG VC] Prechargement natif d'Ocean Beach...", 255, 200, 80)
        return
    end
...
addEvent("ugVcTeleportCommitted", true)
addEventHandler("ugVcTeleportCommitted", resourceRoot, function(token)
...
addEvent("ugVcPrepareCancelled", true)
addEventHandler("ugVcPrepareCancelled", resourceRoot, function(token)
...
addEventHandler("ugWorldDeactivateCities", root, function(nextCity)
    ...
    releaseMap()
    triggerServerEvent("ugVcClientReady", resourceRoot, true, "standby; autre ville residente")
