...
# FileID partition spans double as native pool-loop bounds. This relocation
# therefore keeps TXD/COL/IPL at the capacities installed before the stores
# checkpoint; their final 8,000/512/1,024 spans must move atomically with the
# corresponding pools. DAT remains the stock 64-entry path-node partition.
TARGET_LAYOUT = {
    "DFF": 0,
    "TXD": 32_000,
    "COL": 40_000,
...
# These FLA entries live between the streaming-table pointer block and the
# following marker, but target CD-stream channel buffers rather than
# CStreaming::ms_aInfoForModel. Keep the exclusion exact so a new unsupported
# FileID operand fails generation instead of disappearing silently.
IGNORED_STREAMING_POINTER_PREFIXES = ("&ms_channel[1].readbuf_id[",)
...
# These operands are easy to miss because FLA derives them through local
# variables instead of writing a FileID base directly. They size the backing
# TXD/COL/IPL allocations and every native byte/count loop over those stores.
# The structure sizes are the exact GTA SA 1.0 US layouts used by FLA.
DERIVED_STORE_EXPRESSIONS = {
    "(GetBaseID(FILE_TYPE_COL) - GetBaseID(FILE_TYPE_TXD)) * sizeof(TxdDef)":
        lambda layout: (layout["COL"] - layout["TXD"]) * 12,
    "((GetBaseID(FILE_TYPE_COL) - GetBaseID(FILE_TYPE_TXD)) + 3) * sizeof(TxdDef)":
        lambda layout: ((layout["COL"] - layout["TXD"]) + 3) * 12,
    "sizeOfAllCollisionFiles": lambda layout: (layout["IPL"] - layout["COL"]) * 44,
    "GetBaseID(FILE_TYPE_IPL) - GetBaseID(FILE_TYPE_COL)": lambda layout: layout["IPL"] - layout["COL"],
    "sizeOfAllIPLfiles": lambda layout: (layout["DAT"] - layout["IPL"]) * 52,
    "GetBaseID(FILE_TYPE_DAT) - GetBaseID(FILE_TYPE_IPL)": lambda layout: layout["DAT"] - layout["IPL"],
    "(GetBaseID(FILE_TYPE_DAT) - GetBaseID(FILE_TYPE_IPL)) * 24":
        lambda layout: (layout["DAT"] - layout["IPL"]) * 24,
}

STORE_ALLOCATION_EXPRESSIONS = {
    0x731F5F + 1: lambda layout: layout["COL"] - layout["TXD"],
    0x411457 + 1: lambda layout: layout["IPL"] - layout["COL"],
    0x405F25 + 1: lambda layout: layout["DAT"] - layout["IPL"],
}
...
def build_pointer_patches(function: str, image: PeImage) -> list[PointerPatch]:
    ...
    for address, expression in pointer_calls(function[streaming_start:base_start]):
        try:
            ...
            if expression.startswith(IGNORED_STREAMING_POINTER_PREFIXES):
                continue
            raise
...
def build_value_patches(function: str, image: PeImage) -> list[ValuePatch]:
    ...
    for match in re.finditer(r"CPatch::PatchUINT32\((.*?)\);", section, flags=re.DOTALL):
        ...
        resolver = PURE_BASE_EXPRESSIONS.get(expression) or DERIVED_STORE_EXPRESSIONS.get(expression)
        if resolver is None:
            continue
        address = parse_address(arguments[0])
        stock_value = resolver(STOCK_LAYOUT) & 0xFFFFFFFF
        replacement = resolver(TARGET_LAYOUT) & 0xFFFFFFFF
        if stock_value != replacement:
            patches.append(ValuePatch(address, stock_value, replacement, expression))

    limit_section = strip_comments(function[function.index(LIMIT_MARKER) :])
    for address, resolver in STORE_ALLOCATION_EXPRESSIONS.items():
        address_pattern = rf"CPatch::PatchUINT32\(\s*(?:0x{address - 1:X}|0x{address - 1:x})\s*\+\s*1\s*,"
        if not re.search(address_pattern, limit_section):
            raise ValueError(f"missing FLA store-allocation patch at 0x{address:08X}")
        patches.append(ValuePatch(address, resolver(STOCK_LAYOUT), resolver(TARGET_LAYOUT), "store allocation count"))

    by_address: dict[int, ValuePatch] = {}
    for patch in patches:
        previous = by_address.setdefault(patch.address, patch)
        if previous != patch:
            raise ValueError(f"conflicting value patch at 0x{patch.address:08X}")
        if dword(image, patch.address) != patch.expected:
            raise ValueError(f"value operand mismatch at 0x{patch.address:08X}")
...
def build_movzx_patches(int32_header: Path, image: PeImage) -> list[tuple[int, bytes]]:
    ...
    for line in int32_header.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if not match or "movsx" not in match.group(2):
            continue
...
def write_manifest(
    ...
    lines = [
        ...
        "// Covers both low entry code and the active HOODLUM code in MTA's expanded executable.",
        "// Target: 32,000 DFF / 8,000 TXD / 512 COL / 1,024 IPL; DAT and later non-world spans remain stock.",
        "",
    ]
